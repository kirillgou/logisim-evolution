...
public class RamDualPortAppearance {
  ...
  public static int getNrOfPorts(AttributeSet attrs) {
    return getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        ...
        + getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrLEPorts(attrs) + getNrBEPorts(attrs);
  }
  ...
  ////For Dual Port////
  /////////////////////
  public static int getAddrIndexB(int portIndex, AttributeSet attrs) {
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs);
    return (portIndex == 0) ? (portOffset + portIndex) : -1;
  }

  public static int getDataOutIndexB(int portIndex, AttributeSet attrs) {
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs) + getNrAddrPorts(attrs);
    return getDataOffset(portOffset, portIndex, attrs);
  }
  public static int getDataInIndexB(int portIndex, AttributeSet attrs) {
    if (!seperatedBus(attrs)) return getDataOutIndex(portIndex, attrs);
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs) + getNrAddrPorts(attrs)
        + getNrDataOutPorts(attrs);
    return getDataOffset(portOffset, portIndex, attrs);
  }
  public static int getOEIndexB(int portIndex, AttributeSet attrs) {
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs) + getNrAddrPorts(attrs)
        + getNrDataPorts(attrs);
    int nrOEs = getNrOEPorts(attrs);
    if (nrOEs == 0 || portIndex < 0) return -1;
    if (portIndex < nrOEs) return portOffset + portIndex;
    return -1;
  ...
  public static int getWEIndexB(int portIndex, AttributeSet attrs) {
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs) + getNrAddrPorts(attrs)
        + getNrDataPorts(attrs) + getNrOEPorts(attrs);
    int nrWEs = getNrWEPorts(attrs);
    if (nrWEs == 0 || portIndex < 0) return -1;
    if (portIndex < nrWEs) return portOffset + portIndex;
    return -1;
  }
  public static int getLEIndexB(int portIndex, AttributeSet attrs) {
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs) + getNrAddrPorts(attrs)
        + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs);
    int nrLEs = getNrLEPorts(attrs);
    if (nrLEs == 0 || portIndex < 0) return -1;
    if (portIndex < nrLEs) return portOffset + portIndex;
    return -1;
  ...
  public static int getBEIndexB(int portIndex, AttributeSet attrs) {
    int portOffset = getNrAddrPorts(attrs) + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs)
        + getNrClkPorts(attrs) + getNrLEPorts(attrs) + getNrBEPorts(attrs) + getNrClrPorts(attrs) + getNrAddrPorts(attrs)
        + getNrDataPorts(attrs) + getNrOEPorts(attrs) + getNrWEPorts(attrs) + getNrLEPorts(attrs);
    int nrBEs = getNrBEPorts(attrs);
    if (nrBEs == 0 || portIndex < 0) return -1;
    if (portIndex < nrBEs) return portOffset + portIndex;
    return -1;
  ...
  public static void configurePorts(Instance instance) {
    ...
    for (var i = 0; i < getNrDataInPorts(attrs); i++)
      ps[getDataInIndexB(i, attrs)] = getDataInBPort(i, attrs);
    for (var i = 0; i < getNrDataOutPorts(attrs); i++)
      ps[getDataOutIndexB(i, attrs)] = getDataOutBPort(i, attrs);
    for (var i = 0; i < getNrOEPorts(attrs); i++)
      ps[getOEIndexB(i, attrs)] = getOEPortB(i, attrs);
    for (var i = 0; i < getNrWEPorts(attrs); i++)
      ps[getWEIndexB(i, attrs)] = getWEPortB(i, attrs);
    for (var i = 0; i < getNrLEPorts(attrs); i++)
      ps[getLEIndexB(i, attrs)] = getLEPortB(i, attrs);
    for (var i = 0; i < getNrBEPorts(attrs); i++)
      ps[getBEIndexB(i, attrs)] = getBEPortB(i, attrs);
  ...
  public static Bounds getBounds(AttributeSet attrs) {
    ...
    if (classicAppearance(attrs)) {
      int len = Math.max(30, 10 + getNrDataOutPorts(attrs) * 20);
...
//      int len = Math.max(attrs.getValue(Mem.DATA_ATTR).getWidth() * 20, (getNrLEPorts(attrs) + 1) * 10);
      int len = Math.max(30, 10 + getNrDataOutPorts(attrs) * (seperatedBus(attrs) ? 20:10));
      return Bounds.create(0, 0, Mem.SymbolWidth + xoffset, getControlHeight(attrs) + len);
    }
  }
  ...
  public static void drawRamEvolution(InstancePainter painter) {
    ...
    /* draw the contents (matrix inside of the component)*/
    if (painter.getShowState()) {
...
//      if (!classicAppearance(attrs))
        result += 30;
      result += 2*getNrLEPorts(attrs) * 10;
    } else if (attrs.containsAttribute(StdAttr.TRIGGER)) {
      final var async = !synchronous(attrs);
      result += 20;
      if (!async) result += 10;
      result += 2*getNrBEPorts(attrs) * 10;
    }
    return result;
  }
  ...
  private static Port getAddrPort(int portIndex, AttributeSet attrs) {
    final var result = new Port(0, 20, Port.INPUT, attrs.getValue(Mem.ADDR_ATTR));
    result.setToolTip(S.getter("memAddrTip"));
//    System.out.println("Add_A: " + attrs.getValue(MemDualPort.ADDR_ATTR));
    return result;
  }
  private static Port getAddrBPort(int portIndex, AttributeSet attrs) {
//    System.out.println("Add_B: " + attrs.getValue(MemDualPort.ADDR_B_ATTR));
    final var result = new Port(0, 30, Port.INPUT, attrs.getValue(MemDualPort.ADDR_B_ATTR));
    result.setToolTip(S.getter("memAddrTip"));
    return result;
  }
  private static Port getDataInPort(int portIndex, AttributeSet attrs) {
    ...
    var ypos = getControlHeight(attrs) + 10;
...
//      xpos += 10;
  ...
  private static Port getDataInBPort(int portIndex, AttributeSet attrs) {
    final var nrDins = getNrDataInPorts(attrs);
    if (nrDins == 0 || portIndex < 0) return null;
    if (portIndex >= nrDins) return null;
    var ypos = getControlHeight(attrs) + 10 + getNrDataInPorts(attrs)*10;
    final var classic = classicAppearance(attrs);
    final var bits = attrs.getValue(MemDualPort.DATA_B_ATTR);
    if (!classic && bits.getWidth() == 1) ypos += 10;
    ypos += portIndex * 10;
    final var result = new Port(0, ypos, Port.INPUT, bits);
    switch (portIndex) {
  ...
  private static Port getDataOutBPort(int portIndex, AttributeSet attrs) {
    final var nrDouts = getNrDataOutPorts(attrs);
    if (nrDouts == 0 || portIndex < 0) return null;
    if (portIndex >= nrDouts) return null;
    var ypos = getControlHeight(attrs) + 10 + getNrDataInPorts(attrs)*10;
...
//      ypos = 20;
  ...
  private static Port getOEPortB(int portIndex, AttributeSet attrs) {
    final var nrOEs = getNrOEPorts(attrs);
    if (nrOEs == 0 || portIndex < 0) return null;
    if (portIndex >= nrOEs) return null;
    var ypos = 60;
//    if (attrs.getValue(Mem.ENABLES_ATTR).equals(Mem.USELINEENABLES) && classicAppearance(attrs))
//      ypos = 20;
//    if(!seperatedBus(attrs) && attrs.getValue(Mem.ENABLES_ATTR).equals(Mem.USELINEENABLES)){
//      ypos += 10;
//    }
    final var result = new Port(0, ypos + 20, Port.INPUT, 1);
    result.setToolTip(S.getter("ramOETip"));
    return result;
  }
...
//      ypos = 30;
  ...
  private static Port getWEPortB(int portIndex, AttributeSet attrs) {
    final var nrWEs = getNrWEPorts(attrs);
    if (nrWEs == 0 || portIndex < 0) return null;
    if (portIndex >= nrWEs) return null;
    var ypos = 50;
...
//    if (attrs.getValue(Mem.ENABLES_ATTR).equals(Mem.USELINEENABLES) && classicAppearance(attrs))
//      ypos = 40;
    ypos += 2 * getNrLEPorts(attrs) * 10;
    ypos += 2 * getNrBEPorts(attrs) * 10;
//    ypos +=60;
    final var result = new Port(0, ypos, Port.INPUT, 1);
  ...
  private static Port getLEPort(int portIndex, AttributeSet attrs) {
    ...
    var ypos = seperatedBus(attrs) ? 70 : 90;// todo change
//    if (attrs.getValue(Mem.ENABLES_ATTR).equals(Mem.USELINEENABLES) && classicAppearance(attrs))
//      ypos = 40;
    ypos += portIndex * 10;
    final var result = new Port(0, ypos, Port.INPUT, 1);
  ...
  private static Port getLEPortB(int portIndex, AttributeSet attrs) {
    final var nrLEs = getNrLEPorts(attrs);
    if (nrLEs == 0 || portIndex < 0) return null;
    if (portIndex >= nrLEs) return null;
    var ypos = seperatedBus(attrs) ? 70 : 90;
    ypos += nrLEs*10;
//    if (attrs.getValue(Mem.ENABLES_ATTR).equals(Mem.USELINEENABLES) && classicAppearance(attrs))
//      ypos = 40;
    ypos += portIndex * 10;
  ...
  private static Port getBEPortB(int portIndex, AttributeSet attrs) {
    final var nrBEs = getNrBEPorts(attrs);
    if (nrBEs == 0 || portIndex < 0) return null;
    if (portIndex >= nrBEs) return null;
    final var ypos = 70 + (nrBEs - portIndex - 1) * 10;
  ...
  //connection create somewhere else
...
//      System.out.println("Out idx " + idx);
//      if (!classic) {
...
//      System.out.println("Addr idx " + idx);
//      if (!classic) {
//        final var loc = inst.getPortLocation(idx);
//        int x = loc.getX();
//        int y = loc.getY();
...
//      }
    ...
    ////Addr (B)
    for (var i = 0; i < getNrAddrPorts(attrs); i++) {
      label = !classic ? "" : getNrAddrPorts(attrs) == 1 ? "B" : "B" + i;
      int idx = getAddrIndexB(i, attrs);
      painter.drawPort(idx, label, Direction.EAST);
    }
    ////DataInB
    for (var i = 0; i < getNrDataInPorts(attrs); i++) {
      label = !classic ? "" : getNrDataInPorts(attrs) == 1 ? "D_B" : "D_B" + i;
      final var idx = getDataInIndexB(i, attrs);
      painter.drawPort(idx, label, Direction.EAST);
    }
    ////DataOutB
    for (var i = 0; i < getNrDataOutPorts(attrs); i++) {
      label = !classic ? "" : getNrDataOutPorts(attrs) == 1 ? "D_B" : "D_B" + i;
      int idx = getDataOutIndexB(i, attrs);
      painter.drawPort(idx, label, Direction.WEST);
    }

    for (var i = 0; i < getNrOEPorts(attrs); i++) {
      label = !classic ? "" : getNrOEPorts(attrs) == 1 ? "OE" : "OE" + i;
      final var idx = getOEIndexB(i, attrs);
      if (!classic) {
        final var loc = inst.getPortLocation(idx);
        g.drawLine(loc.getX(), loc.getY(), loc.getX() + 20, loc.getY());
      }
    ...
    for (var i = 0; i < getNrWEPorts(attrs); i++) {
      label = !classic ? "" : getNrWEPorts(attrs) == 1 ? "WE" : "WE" + i;
      final var idx = getWEIndexB(i, attrs);
      if (!classic) {
        final var loc = inst.getPortLocation(idx);
        g.drawLine(loc.getX(), loc.getY(), loc.getX() + 20, loc.getY());
      }
    ...
    for (var i = 0; i < getNrLEPorts(attrs); i++) {
      label = !classic ? "" : getNrLEPorts(attrs) == 1 ? "LE" : "LE" + i;
      final var idx = getLEIndexB(i, attrs);
      if (!classic) {
        final var loc = inst.getPortLocation(idx);
        g.drawLine(loc.getX(), loc.getY(), loc.getX() + 20, loc.getY());
      }
  ...
  private static void drawControlBlock(Instance inst, AttributeSet attrs, InstancePainter painter) {
    ...
    for (var i = 0; i < getNrAddrPorts(attrs); i++) {
      ...
      final var loc = inst.getPortLocation(idx);
      GraphicsUtil.drawText(g, "Addr.A", loc.getX() + 33, loc.getY(), GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
      drawAddressSize(g, loc.getX()+35, loc.getY()-12, attrs.getValue(Mem.ADDR_ATTR).getWidth());
    }
    for (var i = 0; i < getNrAddrPorts(attrs); i++) {
      final var idx = getAddrIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      GraphicsUtil.drawText(g, "Addr.B", loc.getX() + 33, loc.getY(), GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    }

    for (var i = 0; i < getNrDataInPorts(attrs); i++) {
      final var idx = getDataInIndex(i, attrs);
      final var loc = inst.getPortLocation(idx);
      GraphicsUtil.drawText(g, "Din.A", loc.getX() + 33, loc.getY(), GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    }
    for (var i = 0; i < getNrDataOutPorts(attrs); i++) {
      final var idx = getDataOutIndex(i, attrs);
      final var loc = inst.getPortLocation(idx);
      if (seperatedBus(attrs))
        GraphicsUtil.drawText(g, "Dout.A", loc.getX()-33, loc.getY(), GraphicsUtil.H_RIGHT, GraphicsUtil.V_CENTER);
      else
        GraphicsUtil.drawText(g, "Dinout.A", loc.getX() + 33, loc.getY(), GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    }
    for (var i = 0; i < getNrDataInPorts(attrs); i++) {
      final var idx = getDataInIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      GraphicsUtil.drawText(g, "Din.B", loc.getX() + 33, loc.getY(), GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    }
    for (var i = 0; i < getNrDataOutPorts(attrs); i++) {
      final var idx = getDataOutIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      if (seperatedBus(attrs))
        GraphicsUtil.drawText(g, "Dout.B", loc.getX()-33, loc.getY(), GraphicsUtil.H_RIGHT, GraphicsUtil.V_CENTER);
      else
        GraphicsUtil.drawText(g, "Dinout.B", loc.getX()-33, loc.getY(), GraphicsUtil.H_RIGHT, GraphicsUtil.V_CENTER);
    }
    ...
    for (var i = 0; i < getNrOEPorts(attrs); i++) {
      final var idx = getOEIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      final var label = "M" + cidx + " [Output enable B]";
      cidx++;
      g.drawString(label, loc.getX() + 33, loc.getY() + 5);
    }
    ...
    for (var i = 0; i < getNrWEPorts(attrs); i++) {
      final var idx = getWEIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      final var label = "M" + cidx + " [Write enable B]";
      cidx++;
      g.drawString(label, loc.getX() + 33, loc.getY() + 5);
    }
    for (var i = 0; i < getNrLEPorts(attrs); i++) {
      final var idx = getLEIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      final var label = "M" + cidx + " [Line enable " + i + "B]";
      cidx++;
      g.drawString(label, loc.getX() + 33, loc.getY() + 5);
    }
    for (var i = 0; i < getNrBEPorts(attrs); i++) {
      final var idx = getBEIndexB(i, attrs);
      final var loc = inst.getPortLocation(idx);
      final var label = "M" + cidx + " [Byte enable " + i + "B]";
      cidx++;
      g.drawString(label, loc.getX() + 33, loc.getY() + 5);
    }
    g.dispose();
...
//    });
    g.setStroke(new BasicStroke(2));
    final var height = 10 + getNrDataOutPorts(attrs) * (seperatedBus(attrs) ? 20 : 10);
    g.drawRect(x, y, width, height);
    g.dispose();
  }
  ...
  private static void drawAddressSize(Graphics2D g, int xpos, int ypos, int nrAddressBits) {
...
//    GraphicsUtil.drawText(g, "A", xpos + 50, ypos + 20, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    ...
    g.drawLine(xpos + 50, ypos + 20, xpos + 50 + StrSize, ypos + 20);
    GraphicsUtil.drawText(g, "0", xpos + 50 + (StrSize / 2), ypos + 22, GraphicsUtil.H_CENTER, GraphicsUtil.V_BOTTOM);
    GraphicsUtil.drawText(g, size, xpos + 50 + (StrSize / 2), ypos + 18, GraphicsUtil.H_CENTER, GraphicsUtil.V_TOP);
  }
